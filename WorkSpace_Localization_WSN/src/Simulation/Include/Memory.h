...
_declspec(dllexport) int fnpFreeMemory_dbg(void* p, size_t size, int line, const char* file);
...
_declspec(dllexport) void netsim_enable_memcheck();
_declspec(dllexport) bool netsim_is_memcheck_enable();
_declspec(dllexport) bool is_all_ptr_valid();
_declspec(dllexport) bool is_ptr_valid(void* p);
